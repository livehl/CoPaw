...
## 更新摘要
**所做更改**
- 新增了技能扫描器的完整规则体系，包括命令注入、数据泄露、硬编码密钥、混淆攻击等威胁检测
- 扩展了工具守卫功能，增强了文件路径守护和规则守护能力
- 完善了控制台安全配置界面，支持更细粒度的安全策略管理
- 增强了安全扫描的威胁检测能力和误报控制机制
...
## 简介
本文件系统性梳理 CoPaw 的安全增强机制，重点覆盖两大子系统：技能包静态扫描（Skill Scanner）与工具调用前置防护（Tool Guard）。新增的技能扫描器规则体系涵盖了命令注入、数据泄露、硬编码密钥、混淆攻击等多种安全威胁的检测能力，而工具守卫功能则提供了更强大的文件路径验证和规则匹配能力。两个子系统相互独立、可独立启用/禁用，并通过统一的配置中心与控制台界面进行管理。
...
RS["规则签名系统<br/>8种威胁类型"]
CI["命令注入规则<br/>command_injection.yaml"]
DE["数据泄露规则<br/>data_exfiltration.yaml"]
HS["硬编码密钥规则<br/>hardcoded_secrets.yaml"]
OB["混淆攻击规则<br/>obfuscation.yaml"]
end
subgraph "工具防护子模块"
...
### 规则签名系统（新增）
技能扫描器现在集成了完整的威胁检测规则体系，涵盖以下8种主要威胁类型：

#### 命令注入检测（Command Injection）
- **危险函数检测**：eval()、exec()、compile()等动态代码执行函数
- **Shell命令注入**：os.system()、subprocess模块的不安全使用
- **路径遍历**：open()函数与用户输入结合的路径遍历风险
- **SQL注入**：f-string SQL查询中的变量注入风险
- **JavaScript执行**：child_process模块和Function构造器的动态执行

#### 数据泄露检测（Data Exfiltration）
- **网络请求监控**：requests、httpx、aiohttp等网络库的可疑使用
- **敏感文件访问**：/etc/passwd、/etc/shadow、.ssh目录等系统敏感文件
- **Base64编码传输**：数据编码与网络传输的组合模式
- **JavaScript文件系统**：Node.js fs模块的敏感文件操作

#### 硬编码密钥检测（Hardcoded Secrets）
- **AWS密钥**：AKIA开头的AWS访问密钥
- **API令牌**：Stripe、GitHub等服务的API密钥
- **私钥检测**：完整的私钥块格式识别
- **连接字符串**：数据库连接字符串中的硬编码凭据

#### 混淆攻击检测（Obfuscation）
- **Base64解码执行**：编码后的恶意代码执行链
- **十六进制编码**：大量hex blob的可疑模式
- **XOR编码**：XOR混淆与解码执行的组合
- **二进制文件**：可执行二进制文件的引入风险

```mermaid
classDiagram
class RuleSignatureSystem {
+command_injection_rules : list
+data_exfiltration_rules : list
+hardcoded_secrets_rules : list
+obfuscation_rules : list
+prompt_injection_rules : list
+social_engineering_rules : list
+supply_chain_rules : list
+unauthorized_tool_use_rules : list
+load_all_rules() list
...
RuleSignatureSystem --> ThreatCategory
```
...
- 技能扫描
  ...
  - **新增**：规则签名系统采用高效的正则表达式匹配算法，支持规则缓存和智能加载
- 工具防护
...
## 结论
本安全增强方案通过"静态扫描 + 前置防护"的双轨机制，有效降低了技能安装与工具执行过程中的安全风险。新增的完整规则签名系统为技能扫描提供了全面的威胁检测能力，涵盖命令注入、数据泄露、硬编码密钥、混淆攻击等多种安全威胁。工具防护引擎的增强功能进一步提升了文件路径验证和规则匹配的准确性。策略化与可配置的设计使得组织能够根据自身合规要求灵活调整检测强度与范围；前后端一体化的管理界面进一步提升了运维效率。建议在生产环境中结合业务场景持续优化规则集与阈值，并定期审查被阻止历史与白名单，以实现安全与可用性的平衡。

## 附录
- 关键术语
  - 严重性等级：CRITICAL/HIGH/MEDIUM/LOW/INFO/SAFE
  - 威胁类别：命令注入、数据泄露、路径穿越、敏感文件访问、网络滥用、权限提升、混淆攻击、硬编码密钥等
- 常用配置项
  - 工具防护：enabled、guarded_tools、denied_tools、custom_rules、disabled_rules
  - 技能扫描：策略文件、文件分类、文件限制、规则覆盖与禁用
  - **新增**：规则签名系统配置、威胁检测阈值、误报控制参数