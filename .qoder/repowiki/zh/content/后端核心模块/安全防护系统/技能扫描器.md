...
## 更新摘要
**变更内容**
- 新增scanError工具，提供错误解析和模态显示功能
- 增强技能管理安全性，改进错误处理和用户体验
- 新增警告模式下的扫描结果检查功能
- 完善扫描错误的用户界面展示
...
4. [架构总览](#架构总览)
5. [详细组件分析](#详细组件分析)
6. [新增功能：scanError工具](#新增功能scanerror工具)
7. [依赖分析](#依赖分析)
8. [性能考虑](#性能考虑)
9. [故障排查指南](#故障排查指南)
10. [结论](#结论)
...
- 规则：按威胁类别组织的YAML签名集合
- **新增**：scanError工具：提供错误解析、模态显示和警告检查功能
...
- [console/src/utils/scanError.ts:11-173](file://console/src/utils/scanError.ts#L11-L173)
...
- **新增**：scanError工具：提供错误解析、模态显示、警告检查和历史记录功能
...
## 架构总览
下图展示从应用侧到扫描器内部的调用链路与关键对象交互，包括新增的scanError工具集成。
...
participant ErrorTool as "scanError工具"
...
alt 结果不安全且为block模式
API->>ErrorTool : 解析错误并显示模态
ErrorTool-->>App : 显示安全扫描错误详情
else 结果为warn模式
API->>ErrorTool : 检查警告并显示模态
ErrorTool-->>App : 显示警告扫描结果
end
API-->>App : 返回结果或抛出阻断异常
```
...
### 功能概述
scanError工具是一个专门用于处理技能扫描错误的前端工具，提供以下核心功能：
- 错误解析：从异常信息中提取安全扫描失败的JSON数据
- 模态显示：友好的用户界面展示扫描结果和威胁详情
- 警告检查：在成功操作后检查警告模式下的扫描结果
- 历史记录：管理扫描历史和白名单状态

### 核心组件

#### 错误解析器（tryParseScanError）
负责从JavaScript错误中提取安全扫描失败的JSON数据：
- 检查错误实例类型
- 查找消息中的JSON字符串
- 验证type字段为"security_scan_failed"
- 返回标准化的SecurityScanErrorResponse对象

#### 模态显示组件
提供两种模态窗口：
- 错误模态（showScanErrorModal）：显示阻断模式下的扫描错误
- 警告模态（showScanWarnModal）：显示警告模式下的扫描结果

#### 警告检查器（checkScanWarnings）
在技能操作成功完成后，检查是否有警告模式的扫描结果：
- 获取扫描历史和配置
- 过滤最新的警告记录
- 检查技能是否在白名单中
- 显示警告模态
...
```mermaid
flowchart TD
Start(["错误发生"]) --> Parse["tryParseScanError<br/>解析错误"]
Parse --> HasError{"是否为安全扫描错误?"}
HasError --> |是| ShowModal["showScanErrorModal<br/>显示错误模态"]
HasError --> |否| HandleNormal["正常错误处理"]
ShowModal --> Success["错误已处理"]
HandleNormal --> End(["结束"])
Success --> End
```
...
- [console/src/utils/scanError.ts:108-126](file://console/src/utils/scanError.ts#L108-L126)

### 集成点
scanError工具在多个地方被集成使用：

#### 技能管理集成
在useSkills钩子中：
- 错误处理：优先使用handleScanError处理扫描错误
- 警告检查：在技能创建、上传、导入后检查警告
- 直接模态：在Hub技能导入失败时直接显示模态

#### UI组件集成
在SkillScannerSection组件中：
- 警告模态：显示扫描历史中的警告记录
- 历史管理：提供查看、清除扫描历史的功能

**章节来源**
- [console/src/utils/scanError.ts:11-173](file://console/src/utils/scanError.ts#L11-L173)
- [console/src/pages/Agent/Skills/useSkills.ts:30-51](file://console/src/pages/Agent/Skills/useSkills.ts#L30-L51)
- [console/src/pages/Settings/Security/components/SkillScannerSection.tsx:28-82](file://console/src/pages/Settings/Security/components/SkillScannerSection.tsx#L28-L82)
...
- **新增**：scanError工具依赖：Modal组件、React渲染、i18n国际化、安全扫描API类型
...
API --> Cache["扫描缓存"]
API --> History["阻断历史持久化"]
ErrorTool["scanError工具"] --> UI["模态显示组件"]
ErrorTool --> I18n["国际化翻译"]
ErrorTool --> API["安全扫描API"]
```
...
- [console/src/utils/scanError.ts:1-9](file://console/src/utils/scanError.ts#L1-L9)
...
- [console/src/utils/scanError.ts:29](file://console/src/utils/scanError.ts#L29)
...
- [console/src/utils/scanError.ts:141-173](file://console/src/utils/scanError.ts#L141-L173)

## 结论
CoPaw技能扫描器以"轻量、可扩展、策略驱动"为核心设计，当前以YAML正则签名为基础，结合策略化规则作用域、严重性覆盖与凭证抑制，形成可定制的静态威胁检测能力。通过编排器的并发与缓存机制，兼顾性能与可维护性。

**新增的scanError工具显著增强了系统的安全性与用户体验**：
- 提供结构化的错误解析和用户友好的模态显示
- 支持阻断模式和警告模式的不同处理策略
- 自动检查扫描历史，确保用户不会遗漏重要的安全警告
- 通过国际化支持多语言环境

未来可按需扩展行为/LLM分析器，保持与现有编排器的解耦，同时继续完善错误处理和用户界面体验。
...
- 应用侧调用
  ...
  - **新增**：使用scanError工具处理扫描错误，提供更好的用户体验
- UI与配置
  - 通过前端useSkillScanner与security API管理扫描模式、超时、白名单与阻断历史
  - **新增**：通过scanError工具检查警告模式的扫描结果
...
**章节来源**
- [console/src/pages/Agent/Skills/useSkills.ts:30-51](file://console/src/pages/Agent/Skills/useSkills.ts#L30-L51)
- [console/src/pages/Agent/Skills/useSkills.ts:189-192](file://console/src/pages/Agent/Skills/useSkills.ts#L189-L192)
- [console/src/utils/scanError.ts:141-173](file://console/src/utils/scanError.ts#L141-L173)